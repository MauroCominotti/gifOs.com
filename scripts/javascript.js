...
window.onload = function () {
    ...
    document.getElementById('createGifs').addEventListener('click', function () {
        document.getElementById('textInput').value = "";//limpio el input de texto
        getSuggestions();
        if (matches[1].classList.contains('d-none')) { //estamos en "mis gifos"
            toggleDisplay(matches[7]);
            toggleDisplay(matches[0]);
            if (!matches[6].classList.contains('d-none')) {
                toggleDisplay(matches[6]);
            }
        }
        else { //estamos en la pantalla principal
            if (!matches[6].classList.contains('d-none')) { //estamos en busquedas

                toggleDisplay(matches[0]);//cierro busqueda
                toggleDisplay(matches[1]);
                toggleDisplay(matches[6]);
                toggleDisplay(matches[7]);//abro crear gifs
                toggleDisplay(matches[8]);//abro mis gifos
            }
            else {
                for (i = 0; i < matches.length; i++) {
                    if (i !== 6) //cierro todo menos busquedas que ya lo está
                        toggleDisplay(matches[i])
                }
            }
        }
    ...
    document.getElementById('cancelGif').addEventListener('click', function () { for (i = 0; i < matches.length; i++) { if (i !== 6) { toggleDisplay(matches[i]) } } });
